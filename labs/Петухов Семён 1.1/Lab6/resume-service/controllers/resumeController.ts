...
import { publishResumeCreatedEvent } from "../libs/messageQueue";
...
// Пробрасываем Authorization из исходного запроса, чтобы user-service пропустил проверку
const checkUserExists = async (userId: number, authHeader?: string): Promise<boolean> => {
    try {
        await axios.get(
            `http://user-service:3001/user-service/user/${userId}`,
            authHeader
...
export const createResume = async (req: Request, res: Response) => {
    ...
    // Проверка пользователя (с пробросом токена, если он есть)
    const authHeader = req.headers.authorization;
    const userExists = await checkUserExists(userId, authHeader);
    if (!userExists) {
...
export const updateResume = async (req: Request, res: Response) => {
    ...
    if (req.body.userId !== undefined) {
        const authHeader = req.headers.authorization;
        const userExists = await checkUserExists(req.body.userId, authHeader);
        if (!userExists) {
