...
export class MessageService implements IMessageService {
    ...
    async post(authorId: number, entity: DeepPartial<CreateMessageDto>, authToken: string)
    {
        const userIs = await this._userService.getUser(authToken, authorId)
        console.log(userIs)
        if (userIs != 200) {
            throw new HttpError(userIs);
        }
