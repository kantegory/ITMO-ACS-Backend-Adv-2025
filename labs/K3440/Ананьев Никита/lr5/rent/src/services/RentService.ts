...
import { RentPublisher } from '../infrastructure/rentPublisher'
...
export class RentService implements IRentService {
    constructor(
        ...
        @Inject('rent.publisher')
        private readonly rentPublisher: RentPublisher
    ) {}
    ...
    async startRent(rentData: CreateRentDto): Promise<ResponseRentDto> {
        ...
        try {
            ...
            await this.rentPublisher.rentCreated({
                inner: `[Служебное сообщение] Сделка №${rent.id} заключена - договор об аренде вступает в силу.`,
